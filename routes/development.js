...
router.get('/', ensureLoggedIn, function(req, res, next) {
	...
	conn.query('SELECT first_name ,last_name  FROM dept_manager d LEFT JOIN employees e ON e.emp_no=d.emp_no WHERE dept_no = ?',"d005",function(err, rows){
		if(err) {
			console.log(err);
			res.render('error', {error: 'Problem Occured'});
			} else {
				if(rows.length >0){
					res.render('development', {d: rows});
				}
...
router.post('/search', ensureLoggedIn, function(req, res, next) {
	...
	var dep="Development";
	var query="SELECT * FROM dept_emp de LEFT JOIN departments ON departments.dept_no=de.dept_no LEFT JOIN employees e ON e.emp_no=de.emp_no WHERE e.last_name ='"+emp_name+"' and dept_name='development' ";
	console.log("query = "+ query);
	conn.query("SELECT e.emp_no,first_name,last_name,birth_date,gender,hire_date,departments.dept_no,departments.dept_name,from_date,to_date FROM dept_emp de LEFT JOIN departments ON departments.dept_no=de.dept_no LEFT JOIN employees e ON e.emp_no=de.emp_no WHERE e.first_name ='"+emp_name+"' and dept_name='development' limit 1",function(err, rows){
