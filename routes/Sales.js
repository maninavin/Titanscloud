...
router.get('/', ensureLoggedIn, function(req, res, next) {
	...
	conn.query('SELECT first_name ,last_name  FROM dept_manager d LEFT JOIN employees e ON e.emp_no=d.emp_no WHERE dept_no = ?',"d007",function(err, rows){
		if(err) {
			console.log(err);
			res.render('error', {error: 'Problem Occured'});
			} else {
				if(rows.length >0){
					res.render('Sales', {d: rows});
				}
